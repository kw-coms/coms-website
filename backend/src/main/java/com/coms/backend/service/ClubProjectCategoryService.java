...
@Service
@Transactional
public class ClubProjectCategoryService {

    private final ClubProjectCategoryRepository categoryRepository;
    private final ClubProjectRepository projectRepository;

    public ClubProjectCategoryService(ClubProjectCategoryRepository categoryRepository,
    ...
    public List<ClubProjectCategoryResponse> list() {
        Map<String, Long> counts = projectRepository.countByCategory().stream()
    ...
    public ClubProjectCategoryResponse create(ClubProjectCategoryRequest request) {
        ...
        ClubProjectCategory saved = categoryRepository.save(category);
        return toResponse(saved, 0L);
    }

    public ClubProjectCategoryResponse update(Long id, ClubProjectCategoryRequest request) {
        ClubProjectCategory category = categoryRepository.findById(id)
        ...
        ClubProjectCategory saved = categoryRepository.save(category);
        long count = projectRepository.countByCategory(saved.getKey());
    ...
    public void delete(Long id) {
        ClubProjectCategory category = categoryRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "분류를 찾을 수 없습니다."));
        long inUse = projectRepository.countByCategory(category.getKey());
        if (inUse > 0) {
            throw new ResponseStatusException(HttpStatus.CONFLICT,
                    "이 분류를 사용하는 프로젝트가 " + inUse + "건 있어 삭제할 수 없습니다. 먼저 다른 분류로 변경하세요.");
        }
    ...
    public void requireValidKey(String key) {
        if (!categoryRepository.existsByKey(key)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "유효하지 않은 프로젝트 분류입니다.");
        }
    }
    ...
    public Map<String, String> keyToNameMap() {
        return categoryRepository.findAllByOrderByPositionAscIdAsc().stream()
                .collect(Collectors.toMap(ClubProjectCategory::getKey, ClubProjectCategory::getName,
                        (a, b) -> a, java.util.LinkedHashMap::new));
    }

    private int nextPosition() {
        return categoryRepository.findAllByOrderByPositionAscIdAsc().stream()
                .mapToInt(ClubProjectCategory::getPosition)
                .max()
                .orElse(-1) + 1;
    }
    ...
    private ClubProjectCategoryResponse toResponse(ClubProjectCategory category, long projectCount) {
        return new ClubProjectCategoryResponse(
                category.getId(),
                category.getKey(),
                category.getName(),
