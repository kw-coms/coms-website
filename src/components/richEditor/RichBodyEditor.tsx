      ...
      <div className="space-y-3 border-b border-[var(--app-hairline)] bg-[var(--app-surface)] px-3 py-3">
        ...
        <div className="rounded-xl border border-[var(--app-accent-border)]/15 bg-[#f7f9ff] p-3">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            ...
            <button type="button" onClick={insertPollBlock} className="inline-flex min-h-10 items-center justify-center gap-1 rounded bg-[var(--app-brand)] px-3 text-sm font-bold text-white shadow-sm hover:bg-[var(--app-brand-strong)]">
              <Plus size={14} /> 본문에 추가
            </button>
          </div>
          <input
          ...
          <div className="space-y-2">
            {pollOptionInputs.map((option, index) => (
              <div key={index} className="grid gap-2 rounded-lg border border-[var(--app-hairline)] bg-white/70 p-2 sm:grid-cols-[auto_1fr_1fr_auto] sm:items-center">
                <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-[var(--app-brand)] text-xs font-black text-white">{index + 1}</span>
                <input
                  type="text"
                  value={option.label}
